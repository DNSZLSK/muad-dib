...
- **`muaddib evaluate --corpus-dir <dir>` — offline re-scan of a local archive at HEAD.** Measures detection on YOUR own rapatriated captures (e.g. the monitor tarball archive rsync'd to a workstation) by **re-scanning every tarball at the current code** — never a replay of stored scores, unlike the frozen Datadog/OSSF benchmarks. Reads the `src/monitor/tarball-archive.js` layout (`<name>-<version>.tgz` + sidecar `.json`); labels are optional (sidecar `"label"` or a top-level `labels.json`) — with labels it reports **TPR + FPR with Wilson CIs, time-stratified** into `fresh` (≤60d, never rule-tuned — the generalization proxy), `current` (2025→−60d), and `historical`; without labels, score distribution + flagged rate only (honest: the archive stores a score, not a verdict). Bounded (`MUADDIB_CORPUS_MAX`, default 5000; each archive extracted to a per-invocation `mkdtemp` scratch and removed after its scan). Output to `metrics/` or `MUADDIB_CORPUS_OUT_DIR`. Skips the 8 static-corpus phases entirely. Tests in `tests/integration/evaluate-corpus-dir.test.js`.
- **GitHub Action hardening (`action.yml` + `scripts/action-report.cjs`).** The composite Action was double-scanning (once `--json`, once for human output — a real cost on a CPU-bound scanner) and its `threats_count` output was always `1` (`grep -o '"threats":['` counted the array-open token, not the elements). Replaced with a single authoritative `--json` scan parsed by a crash-safe Node reporter that emits correct `risk_score`/`risk_level`/`threats_count`/`critical_count`/`high_count`, prints a severity-ranked capped summary, and appends a **prefilled "false positive" GitHub-issue URL** (advisory-only community feedback loop — never changes a verdict). A second pass runs only when SARIF is requested. Fixed the README example (`@v1` was a dead tag — releases are full `vX.Y.Z`). Tests in `tests/integration/action-report.test.js`.
- **Corpus offload: safe-delete archive retention + workstation pull tooling.** For the "VPS captures / workstation stores the corpus" loop: `cleanupOldArchives` (`src/monitor/tarball-archive.js`) is now **pull-aware** — an aged un-pulled day-dir holding irreplaceable tarballs is **kept** for the pull, dropped only past a hard ceiling (`MUADDIB_ARCHIVE_MAX_RETENTION_DAYS`, default 30 — bounded disk) or under real disk pressure (oldest-first; benign is already JSON-only via the score-gate), never on the soft timer. A `.pulled` sentinel (written back by the workstation after a hash-verified copy) authorises soft-timer purge. Ships `scripts/ops/pull-archive.sh` (+ README) — an env-driven, secret-free template that pulls, verifies each tarball against its sidecar `tarball_sha256`, marks `.pulled`, then reclaims the VPS. Tests in `tests/unit/tarball-archive.test.js`.

### Changed

- **GAP-2 — two dead OSV IOC sources removed (audit 2026-07; fictional coverage).** `scrapeGitHubAdvisory` (deep `muaddib scrape` path) and `scrapeOSVLightweightAPI` (light `muaddib update` path) both POSTed `api.osv.dev/v1/query` with `{ package: { ecosystem: 'npm' } }` — **no package name**, which the OSV `/v1/query` API rejects with **HTTP 400 on every call** (it requires `package.name`+version, a commit, or a purl). Both therefore returned 0 packages permanently while logging as normal completions (`+0 OSV API` in `muaddib update`); the light-path feed-health alarm even had a comment rationalising the constant zero as "public and volatile … rarely crosses baseline" rather than recognising the 400. **Verified no coverage is lost before removing:** (1) `scrapeOSVLightweightAPI` was 100% redundant with the working OSV MAL- zip dump (`scrapeOSVDataDump`, same MAL- npm set); (2) an empirical cross-check of GHSA npm malware advisories published before the local IOC snapshot found **153/153 (100%) already present in the IOC store** via the OSV MAL- dump — the GHSA-malware `scrapeGitHubAdvisory` targeted is fully covered without it, and the GHSA poller (`src/ioc/ghsa-poller.js`) independently tracks the coverage denominator (Phase 5 coverage-audit). Removed both functions, their `runScraper`/`updater.js` wiring, the misleading logs + feed-health comment, and the mocked happy-path tests that validated a 200 response the source never actually returns. `muaddib update` keeps its recent-malware coverage via GenSecAI + DataDog + OSM; the OSV MAL- set comes from the deep `muaddib scrape`. Docs updated (ARCHITECTURE, README.fr source lists). No detection change (both sources contributed 0).
...
- **M5 — monitor systemd hardening brought to `muaddib-scrape.service` parity (audit 2026-07).** `deploy/muaddib-monitor.service` lacked the sandboxing directives already present on `muaddib-scrape.service`. Verifying the gap directive-by-directive corrected the audit on two counts: it had **invented `CapabilityBoundingSet`** (not actually on `scrape.service`, so no parity basis — excluded) and had **missed four real gaps** (`ProtectKernelTunables`, `ProtectControlGroups`, `RestrictAddressFamilies`, `RestrictNamespaces`). Real gap = **9 directives**. Because the monitor — unlike the oneshot scraper — launches the Docker sandbox (`docker run` → `/var/run/docker.sock`), the additions were tiered by container-path risk and **applied + validated on the production VPS one tier at a time (not just in theory)**: Tier A (`LockPersonality`, `RestrictRealtime`, `ProtectKernelModules`, `ProtectKernelTunables`, `PrivateDevices`, `SystemCallArchitectures=native`) → Tier B (`RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6`, `ProtectControlGroups`) → Tier C (`RestrictNamespaces`), with a full sandbox run (container + gVisor) and a registry fetch confirmed working after each restart. Container isolation is `dockerd`'s, so these restrict only the `node` + `docker`-CLI processes, not the containers; none touches the existing OOM memory guards (`MemoryHigh` / `MemoryMax` / `MemorySwapMax`) — `ProtectControlGroups` does not weaken `MemoryMax` (systemd enforces it on the cgroup, not the service process). **Out-of-scope finding, checked and benign:** while confirming `ReadWritePaths` against the top-level `/opt/muaddib/archive` write path (`src/monitor/tarball-archive.js:19`), a read-only VPS check confirmed the archive is already correctly configured — `ProtectSystem=strict` is not blocking tarball writes, nothing to fix.
- **F2 — synchronous CVE gate on PRs and releases (audit 2026-07).** Verified against the real workflows (audit correct here): `publish.yml` ran only `npm audit signatures` (SLSA provenance), not `npm audit` (vulnerabilities), and `scan.yml` (the `pull_request` CI) had no dependency-CVE check at all — its `scan` job is a MUAD'DIB self-scan for malware patterns, not a CVE audit. No equivalent mechanism existed elsewhere: `scorecard.yml` is a scheduled/push scoring run (no `pull_request` trigger, non-blocking), `regression-check.yml` fires only on `metrics/*.json` changes, and there is no pre-commit / pre-push hook. So a vulnerable transitive dependency was surfaced only by weekly Dependabot, never blocked at PR or release time. Added `npm audit --audit-level=high` as a **blocking gate in both** the PR CI (`scan.yml` `test` job, after the typosquat guard) and the release path (`publish.yml`, alongside the existing provenance gate). **Threshold `high`** blocks the actionable HIGH/CRITICAL advisories (RCE, prototype pollution) without false-positive noise from moderates/lows on this project's small dependency tree; `moderate` was rejected as noisier and because it would block on the current unexploitable-in-context finding. **Fail-closed by design:** `timeout-minutes: 5` bounds a sustained npm-advisory-endpoint outage to a fast, predictable failure (better to block a merge/release than to ship un-audited); transient blips are absorbed by npm's built-in `fetch-retries=2` (10–60s backoff), which is more robust than a hand-rolled shell retry. **Honest note:** both gates **pass today** (exit 0) — the sole current advisory is a *moderate* (`brace-expansion` GHSA-jxxr-4gwj-5jf2, a ReDoS below the `high` threshold), left deliberately un-blocked; `npm audit fix` resolves it but edits `package-lock.json`, tracked as a separate decision rather than folded into this gate.
