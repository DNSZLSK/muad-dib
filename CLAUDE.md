...
```bash
npm test          # Run all tests (custom framework, 4545 tests across 153 files)
...
## Current Metrics (v<!--stat:version-->2.11.180<!--/stat:version-->; detection metrics last fully measured v2.11.48)

| Metric | Value |
|--------|-------|
| Version | **<!--stat:version-->2.11.180<!--/stat:version-->** |
| Tests | **4535** passed, **5 failed** locally (Windows-only env flakes — `tar --force-local` ×3 EXTRACT-POOL, temp-file `UNKNOWN` ×2 PACKAGE; **0 failed on Linux/CI**), across **<!--stat:testFiles-->153<!--/stat:testFiles-->** files (14511 skipped when Docker absent). Total non-skipped: <!--stat:tests-->4545<!--/stat:tests-->. |
| Rules | **<!--stat:rulesTotal-->278<!--/stat:rulesTotal-->** (<!--stat:rulesCore-->273<!--/stat:rulesCore--> RULES + <!--stat:rulesParanoid-->5<!--/stat:rulesParanoid--> PARANOID - v2.11.67/70 Phantom Gyp adds PKG-023 `gyp_command_exec` + COMPOUND-017 `gyp_phantom_exec`; 2026-07 anti-evasion adds AST-095 `anti_analysis_evasion` + AST-096 `analyzer_honeytoken_reference`) |
...
- ~~**Cap PyPI à 35/100**~~: ✅ **closed v2.11.84 + Track E (2026-07-23)**. The "cap" was MT-1 (no-lifecycle score ceiling, `scoring.js` §7b) — PyPI packages emit no npm `lifecycle_script`, so everything without a bypass was clamped to 35. v2.11.84 added the `PYPI_IMPORT_TIME_RCE_TYPES` bypass (import-time RCE = the PyPI lifecycle-equivalent). Track E closes the last capped malware shape: **CRITICAL `ioc_string_match` now bypasses MT-1** (campaign-unique strings, decisive by construction — the hash/name IOC types already escaped via `applySingleFireCriticalFloor`; string IOCs had no escape). GT-099 eth-security-auditor: 35 → 100. Measured **0** `ioc_string_match` across 869 benign packages (545 curated + 200 random npm + 124 PyPI) → zero corpus FP cost; repo self-scan unchanged (23). Local re-scan of the 12 v2.11.48 PyPI FPs at HEAD: 7 now score 0–2 (fixed by FP-cap work since v2.11.48); the 4 still ≥20 (django 35, matplotlib 25, bokeh 25, pymongo 35) are real heuristic clusters, not cap artifacts (uncapped scores nearly identical). Official FPR PyPI number: to refresh at the next full `muaddib evaluate` run only.
- **plotly OOM (scanner memory)**: `muaddib scan` on plotly 6.7.0 (cached benign-pypi corpus) now dies at the 4 GB Node heap limit — it was scannable at v2.11.48 (scored 25). Memory regression on giant JS-bundle packages, independent of scoring — to investigate separately.
