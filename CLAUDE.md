...
```bash
npm test          # Run all tests (custom framework, 4550 tests across 151 files)
...
## Current Metrics (v<!--stat:version-->2.11.166<!--/stat:version-->; detection metrics last fully measured v2.11.48)

| Metric | Value |
|--------|-------|
| Version | **<!--stat:version-->2.11.166<!--/stat:version-->** |
| Tests | **4535** passed, **5 failed** locally (Windows-only env flakes — `tar --force-local` ×3 EXTRACT-POOL, temp-file `UNKNOWN` ×2 PACKAGE; **0 failed on Linux/CI**), across **<!--stat:testFiles-->151<!--/stat:testFiles-->** files (14511 skipped when Docker absent). Total non-skipped: <!--stat:tests-->4550<!--/stat:tests-->. |
| Rules | **<!--stat:rulesTotal-->276<!--/stat:rulesTotal-->** (<!--stat:rulesCore-->271<!--/stat:rulesCore--> RULES + <!--stat:rulesParanoid-->5<!--/stat:rulesParanoid--> PARANOID - v2.11.67/70 Phantom Gyp adds PKG-023 `gyp_command_exec` + COMPOUND-017 `gyp_phantom_exec`; 2026-07 anti-evasion adds AST-095 `anti_analysis_evasion` + AST-096 `analyzer_honeytoken_reference`) |
| Scanners | **<!--stat:scanners-->21<!--/stat:scanners--> parallel** (Promise.allSettled) + **2 pre-analysis** (module-graph/, deobfuscate) + **1 async parser bootstrap** (python-ast WASM init, no analysis emitted) + **6 conditional/post-processing** (paranoid, 3× temporal-*, reachability, phantom-gyp) + **1 metadata** (npm-registry). <!--stat:scannerFiles-->35<!--/stat:scannerFiles--> fichiers `src/scanner/*.js` + 3 dirs : `ast-detectors/` (<!--stat:astDetectors-->15<!--/stat:astDetectors-->), `module-graph/` (<!--stat:moduleGraph-->9<!--/stat:moduleGraph-->), `python-ast-detectors/` (<!--stat:pythonAstDetectors-->6<!--/stat:pythonAstDetectors--> fichiers). Détails : ARCHITECTURE.md. |
