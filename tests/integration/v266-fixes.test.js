...
`);
  ...
  // Rule-count drift gate: derive the expected counts from stats.json (the single
  // source of truth, regenerated by `npm run docs:stats`) instead of a hand-edited
  // literal that broke on every legitimate rule addition. This still fails if the
  // live RULES/PARANOID_RULES drift from the committed stats, keeping the doc↔code
  // gate — but a new rule + `npm run docs:stats` updates one place, not two.
  test('P3: RULES / PARANOID_RULES counts match stats.json (drift gate)', () => {
    const { RULES, PARANOID_RULES } = require('../../src/rules/index.js');
    const stats = require('../../stats.json');
    const ruleCount = Object.keys(RULES).length;
    const paranoidCount = Object.keys(PARANOID_RULES).length;
    assert(ruleCount === stats.rulesCore, `RULES count ${ruleCount} != stats.rulesCore ${stats.rulesCore} — run npm run docs:stats`);
    assert(paranoidCount === stats.rulesParanoid, `PARANOID count ${paranoidCount} != stats.rulesParanoid ${stats.rulesParanoid} — run npm run docs:stats`);
  });
