...
async function runEventLoopMonitorTests() {
  ...
  // NB: the former 'LOOP-MON: …compact:scan-ledger…' case was removed with the
  // scan-ledger breadcrumb — that compaction is now ASYNC/non-blocking
  // (fix/async-scan-ledger-compaction), so it can no longer wedge the loop and needs
  // no stall attribution. Its replacement (async + no-append-loss under concurrency)
  // lives in tests/unit/scan-ledger.test.js. The maint:* and compact:detections/
  // compact:temporal-detections breadcrumbs remain (those compactions stay sync).
}
